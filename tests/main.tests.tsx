...
if (Meteor.isClient) {
  describe('dashboard component', function () {
    ...
    it('Dashboard has all Main Content rendered', function () {
      assert.strictEqual(dashboard.find("#taskCreate").length, 1);
      assert.strictEqual(dashboard.find("#taskToDoList").length, 1);
      assert.strictEqual(dashboard.find("#taskDoneList").length, 1);
    })

    it('String Filter can be edited', function () {
      assert.strictEqual(dashboard.find("#SearchStringTodo").length, 1);
      dashboard.find("#SearchStringTodo").simulate('change', {currentTarget: {value: 'MyFilter'}});
      const actual: string = dashboard.find("#SearchStringTodo").prop("value");
      assert.strictEqual(actual, "MyFilter");
    })

    it('Category Filter is default None', function () {
      assert.strictEqual(dashboard.find("#SearchCategoryTodo").length, 1);
      const actual: string = dashboard.find("#SearchCategoryTodo").prop("value");
